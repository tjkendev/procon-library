...
オイラーの $\phi$ 関数 $\phi(m)$ に対し、オイラーの定理より $a^{\phi(m)} \equiv 1$ (mod $m$) を満たすため、 $x \equiv a^{\phi(m)-1}$ を計算することで求まる。

$\phi(m)$ の前計算が必要であるが、 $m$ が素数であれば $\phi(m) = m-1$ であるため、 $x \equiv a^{m-2}$ を計算することで求まる。
