...
- 重さの総和$W$まで入るナップザックに入れる
- ナップザックに入る品物の価値を最大化する

== 0-1ナップザック問題: $O(N^2 \max_i{v_i})$
...
== 個数制限付きナップザック問題: $O(N^2 \max_i{v_i} \max_i{c_i})$
...
----
== 個数制限付きナップザック問題: $O(N^2 \max_i{v_i}^2)$
...
1. 各品物 $m_i = \min(\max_j v_j, c_i)$ 個に絞った個数制限付きナップザック問題を解く
