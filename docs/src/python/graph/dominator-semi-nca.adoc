...
SEMI-NCA では、各頂点 latexmath:[v] の semidominator latexmath:[sdom(v)] し、計算した semidominator を元に徐々に dominator tree を構築する。
...
(競プロ的には、人為的に時間がかかるケースが生成される可能性がある)
...
[source, python]
----
include::./python/graph/dominator-semi-nca.py[]
----

=== Verified

* AOJ: "0294: Catch a Thief": link:https://judge.u-aizu.ac.jp/onlinejudge/review.jsp?rid=6493135#1[source] (Python3, 1.09sec)
